...
| `CRESIUM_PARTNER_SECRET` | Sí (prod) | *Partner secret* de Cresium para HMAC-SHA256 (misma regla que API: `timestamp\|METHOD\|PATH\|BODY` → Base64). Debe ser **exactamente** el secret del partner en Cresium; si el log dice `invalid signature`, revisá este valor. El código prueba variantes de `timestamp` (ISO vs epoch ms/seg) y de path (con/sin `/` inicial). |
