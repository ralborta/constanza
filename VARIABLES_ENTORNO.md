...
**Despliegue en Railway:** hay que tener **un servicio dedicado** que buildee este app (Dockerfile en monorepo). Pasos: [`apps/rail-cucuru/RAILWAY.md`](apps/rail-cucuru/RAILWAY.md).

| Variable | Obligatoria | Descripción |
|----------|-------------|-------------|
| `CRESIUM_TENANT_ID` | Sí | UUID del tenant en Constanza al que se imputan los depósitos. |
| `CRESIUM_PARTNER_SECRET` | Sí (prod) | *Partner secret* de Cresium para HMAC-SHA256. Si el log sigue con `invalid signature`, probá también **`CRESIUM_WEBHOOK_SECRET`** si en el panel hay un secret distinto para webhooks. |
| `CRESIUM_WEBHOOK_SECRET` | No | Si Cresium te da un secret **solo para callbacks/webhooks** (distinto del API partner), ponerlo aquí. Se prueba **antes** que `CRESIUM_PARTNER_SECRET`. |
| `CRESIUM_SIGNATURE_DEBUG` | No | `true`: en logs de firma inválida incluye prefijo del body (cuidado con datos sensibles). |
...
| `CRESIUM_TIMESTAMP_UNIT` | No | `auto` (por defecto): número `x-timestamp` &lt; 1e12 → **segundos** Unix; si no, ms. También se acepta **ISO 8601** (ej. `2026-03-21T03:36:51.000Z`). Forzar: `seconds` o `milliseconds`. |
| `CRESIUM_REPLAY_WINDOW_MS` | No | Ventana anti-replay del timestamp (ms). Por defecto **15 min**; si Cresium reintenta tarde, subir o revisar relojes. |
